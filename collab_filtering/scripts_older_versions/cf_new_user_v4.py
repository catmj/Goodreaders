...
NUM_FEATURES = 300 # IMPORTANT: This must match the NUM_FEATURES used to train/save X
TOP_N_RECOMMENDATIONS = 30 
LAMBDA_NEW_USER = 3.25
...
def train_linear_regression_model(X, Y_mean, test_ratings, lambda_val): # ADDED lambda_val
    """
...
def generate_user_predictions(X_all, Y_mean_all, book_list_final_all, num_books_all, my_books, my_ratings, lambda_new_user): # ADDED lambda_new_user
    """
    ...
    W_user, b_user = train_linear_regression_model(X_all, Y_mean_all, test_ratings_all, lambda_new_user) 
    
    if W_user is None or b_user is None: # Handle case where user model training failed
...
def get_recommendations_for_user(
    ...
    user_predictions_full, test_rated_indices_all, W_user, b_user = \
        generate_user_predictions(X_all, Y_mean_all, book_list_final_all, num_books_all, my_books, my_ratings, lambda_new_user) # PASS lambda
