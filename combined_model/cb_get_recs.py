...
# Call the function to get all books with scores
all_books_with_scores_df = find_all_books_with_scores(
    SIMILARITY_MATRIX_NPY_FILEPATH, 
    BOOK_IDENTIFIERS_TXT_FILEPATH, 
    sample_user_books, 
    sample_user_ratings
)
print(all_books_with_scores_df)

# Example of how you might filter the output:
# Only books not rated by the user, sorted by score:
unrated_books_df = all_books_with_scores_df[~all_books_with_scores_df['Rated by User']]
print("\n--- Unrated Books (Recommended) ---")
print(unrated_books_df.head(10)) # Print top 10 unrated recommendations

# Only books rated by the user:
rated_books_df = all_books_with_scores_df[all_books_with_scores_df['Rated by User']]
print("\n--- Rated Books and Their Scores ---")
print(rated_books_df)