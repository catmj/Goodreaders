...
if __name__ == "__main__":
    ...
    book_features_loaded, item_bias_loaded, all_book_identifiers_loaded, \
    ...
    # Corrected check for None values in loaded data
    if (book_features_loaded is None or item_bias_loaded is None or
        all_book_identifiers_loaded is None or mean_book_ratings_loaded is None or
        total_books_loaded is None):
        print("Failed to load all necessary pre-trained data. Exiting.")
    else:
        # Sample user input (using "Book Identifier" for consistency)
        sample_user_rated_book_identifiers = [
            "the fifth season, n.k. jemisin",
            "all systems red, martha wells",
            "deathless, catherynne m. valente",
            "the city of brass, s.a. chakraborty"
        ]
        sample_user_book_ratings = [5.0, 4.0, 2.0, 5.0]

        # Generate predictions for the user
        full_predictions_df = generate_user_predictions(
            book_features_loaded,
            item_bias_loaded,
            mean_book_ratings_loaded,
            all_book_identifiers_loaded,
            total_books_loaded,
            sample_user_rated_book_identifiers,
            sample_user_book_ratings,
            NEW_USER_REGULARIZATION_STRENGTH
        )

        if full_predictions_df is not None:
            print("\n--- Full List of Books with Predicted Ratings (including user-rated) ---")
            print(full_predictions_df.head(15)) # Display top 15 by predicted rating

            # Example: Filter to see only books the user rated
            user_rated_display_df = full_predictions_df[full_predictions_df['Rated by User']].sort_values(
                by='Original Rating', ascending=False
            )
            print("\n--- Books Rated by User with Original and Predicted Ratings ---")
            print(user_rated_display_df)

            # Example: Filter to see unrated books (recommendations)
            unrated_recommendations_df = full_predictions_df[~full_predictions_df['Rated by User']].head(10)
            print("\n--- Top 10 Recommended Books (Unrated by User) ---")
            print(unrated_recommendations_df)