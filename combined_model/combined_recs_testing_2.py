...
def _get_unrated_books(combined_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        combined_df (pd.DataFrame): The DataFrame containing combined CB and CF scores.
                                   Expected to have a 'Rated_by_User' column (from recs_test_runner.py's aggregation).
    ...
    """
    # FIX: Changed 'Rated by User' to 'Rated_by_User' to match the column name
    # produced by the aggregation in recs_test_runner.py
    unrated_books = combined_df[~combined_df['Rated_by_User']].copy()
...
def _split_user_data_for_testing(user_data_df: pd.DataFrame, validation_split_ratio: float = 0.5) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    maintaining the structure where each row is a user
    and contains lists of their book identifications and ratings.
    ...
    if user_data_df.empty:
        print("Input user_data_df is empty, returning empty training and validation DataFrames.")
    ...
    for index, row in user_data_df.iterrows():
        ...
        if num_items < 2:
            print(f"User {user_id} has fewer than 2 items, skipping split.")
    ...
    print(f"\n--- User Data Split for Testing ---")
    print(f"Total users in original data: {len(user_data_df)}")
    print(f"Users with enough data for split (Training): {len(training_df)}")
    print(f"Users with enough data for split (Validation): {len(validation_df)}")
    ...
    print(f"Total original ratings: {total_original_ratings}")
    print(f"Total ratings in split data: {total_split_ratings}")
...
def _get_relevant_books_from_validation(validation_data_df: pd.DataFrame, relevance_threshold: float = 3.0) -> pd.DataFrame:
    ...
    if validation_data_df.empty:
        print("Validation data DataFrame is empty. Cannot extract relevant books.")
    ...
    print("--- Relevant Books Extraction Complete ---")
...
def get_combined_recommendations(
    ...
    print(f"  User Reg Strength: {new_user_regularization_strength}")
    ...
    if (cb_similarity_matrix_npy_filepath is None or
        ...
        print("Provided loaded_data_tuple indicates failure or incomplete data. Exiting.")
        return None
...
def generate_all_recommendations_for_users(
    ...
    if training_data_df.empty:
        print("Training data DataFrame is empty. Cannot generate recommendations.")
    ...
    for index, user_row in training_data_df.iterrows():
        ...
        if combined_df is None or combined_df.empty:
            print(f"Skipping recommendations for user {user_id} due to empty or failed combined_df.")
            user_recs['Rec_CB_Only'] = []
            user_recs['Rec_CF_Only'] = []
            user_recs['Rec_CB_Filtered_by_CF'] = []
            user_recs['Rec_By_Multiplying_Scores'] = []
        else:
            # Generate recommendations using specific cb_fraction values
...
def evaluate_recommendations_overlap(
    ...
    if merged_df.empty:
        print("Merged DataFrame is empty. No recommendations or relevant books to evaluate.")
        return pd.DataFrame(columns=[
    ...
    # Get the list of recommendation strategy columns dynamically
    # Exclude 'User_id' and ensure we only process recommendation columns
    rec_cols = [col for col in user_recommendations_df.columns if col.startswith('Rec_')]

    for index, row in merged_df.iterrows():
        user_id = row['User_id']
        
        relevant_books_candidate = row['Relevant_Books_List']
        
        # Robustly handle relevant_books_candidate: must be an iterable, not NaN or None
        if relevant_books_candidate is np.nan or relevant_books_candidate is None:
            relevant_books = set()
        elif isinstance(relevant_books_candidate, (list, pd.Series, np.ndarray)):
            # Filter out any potential NaNs within the list/array if they exist
            relevant_books = set(book for book in relevant_books_candidate if pd.notna(book))
        else: # Fallback for any other unexpected scalar type, treat as empty
            relevant_books = set()
        
        result_row = {'User_id': user_id, 'Total_Relevant_Books': len(relevant_books)}

        for rec_col in rec_cols:
            # Ensure recommended_books is treated as an iterable, defaulting to empty if NaN or not list-like
            recommended_books_candidate = row[rec_col]
            if recommended_books_candidate is np.nan or recommended_books_candidate is None:
                recommended_books = set()
            elif isinstance(recommended_books_candidate, (list, pd.Series, np.ndarray)):
                recommended_books = set(book for book in recommended_books_candidate if pd.notna(book))
            else:
                recommended_books = set()

            overlap_count = len(relevant_books.intersection(recommended_books))
            result_row[f'Overlap_{rec_col}'] = overlap_count
        
        evaluation_results.append(result_row)
    
    evaluation_df = pd.DataFrame(evaluation_results)
    print("--- Recommendation Overlap Evaluation Complete ---")
