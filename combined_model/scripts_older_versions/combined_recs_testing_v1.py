...
def evaluate_recommendations_overlap(
    ...
    # Exclude 'User_id' and 'Relevant_Books_List'
    rec_strategy_cols = [col for col in user_recommendations_df.columns if col != 'User_id']

    for index, row in merged_df.iterrows():
        user_id = row['User_id']
        # Handle cases where 'Relevant_Books_List' might be NaN (e.g., no relevant books for user)
        relevant_books_set = set(row['Relevant_Books_List'] if isinstance(row['Relevant_Books_List'], list) else [])
        total_relevant = len(relevant_books_set)

        result_row = {'User_id': user_id, 'Total_Relevant_Books': total_relevant}

        for rec_col in rec_strategy_cols:
            # Handle cases where a recommendation list might be NaN (e.g., if combined_df was empty for a user)
            recommended_books = set(row[rec_col] if isinstance(row[rec_col], list) else [])
            overlap_count = len(recommended_books.intersection(relevant_books_set))
